...
- [x] 添加v1重复输出和v2进程中断的失败测试。
- [x] v1检测已有产物并要求新目录或显式`--overwrite`。
- [x] v2在子进程异常、缺指标或中断时原子写入`failed`及恢复说明。
- [x] 运行v1/v2全套测试和Codex CLI授权边界测试并提交。
...
- [x] 添加测试，拒绝两条只改变同一预算字段的伪多路线。
- [x] 为序列课堂模型增加可执行的上下文长度备选路线；结构化领域在尚无等价路线时标为`design_only`。
- [x] 保证每条路线只改变一个研究变量，最终决策仍使用同一评价器。
- [x] 运行text、protein、weather三条端到端路线并提交。
...
- [x] 添加测试，要求十类领域只从注册表导出，并检查重复注册。
- [x] 将散落的领域常量改为注册表查询，不改变现有命令输出。
- [x] 提供自定义领域模板，明确必须实现读取、表示、预训练目标、下游任务和评价。
- [x] 运行十类smoke、十类classroom和AutoResearch回归测试并提交。
