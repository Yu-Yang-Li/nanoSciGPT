...
- 三个学生入口及其参考材料；
- 讲师编排资料统一放在`docs/instructor/`，根目录仅保留兼容跳转页；
- 十类离线课堂数据与统一CPU入口；
- 运行、比较、停止和证据包所需代码；
- 验收记录与数据来源说明。
...
- 最新 `master` 干净克隆还按完整学生顺序重跑了 LAMOST、文本、Protein、AutoResearch、v1、v2 和证据包；结果为 `AUTORESEARCH_DELTA=0.0224`、`NEXT=stop_branch`、`V2_RETAINED=baseline`，详见同一复现记录。
