...
output = root / "out/native-coder-stream-20260905/run"
...
record = {"status": "started", "model": "openai/scnet/GLM-5.3", "data_identity": "handwritten teaching protein labels",
          ...
          "stream": True, "parent_wall_timeout_seconds": 300}
...
try:
    ...
    def capture_completion(messages, functions, stream, temperature=None):
        ...
        def capture_chunks():
            # Preserve content for code review, but not internal reasoning text.
            with (output / "stream.jsonl").open("w", encoding="utf-8") as log:
                for chunk in response:
                    item = chunk.model_dump()
                    for choice in item.get("choices", []):
                        delta = choice.get("delta") or {}
                        for field in ("reasoning_content", "reasoning", "thinking", "provider_specific_fields"):
                            value = delta.pop(field, None)
                            if value is not None:
                                delta[field + "_characters"] = len(str(value))
                    log.write(json.dumps({"elapsed_seconds": round(time.monotonic() - start, 3), "chunk": item}) + "\n")
                    log.flush()
                    yield chunk
        return response_hash, capture_chunks()
    ...
    coder = Coder.create(main_model=model, fnames=[str(output / n) for n in ("experiment.py", "plot.py", "notes.txt")],
                         read_only_fnames=[str(output / "task_setup.json")], io=io, stream=True, use_git=False,
