...
4. `autoresearch-model-iteration`：待从现有研究循环中拆出。
5. `ai-scientist-v1-workflow`：待从现有研究循环中拆出。
6. `ai-scientist-v2-tree-search`：待补齐多路线比较能力后建立。

现有 [`ai-scientist-research-loop`](ai-scientist-research-loop/SKILL.md) 暂时承接后三段，待对应代码和测试完成后移除。所有 Skill 都沿用学生已经提供的信息，每轮推进一项可检查的操作。课程数据与来源见 [`data/manifest.json`](../data/manifest.json)。
