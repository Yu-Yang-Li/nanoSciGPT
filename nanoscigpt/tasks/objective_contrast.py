"""A2: train the same encoder with CLM vs MLM on the same domain data.
...
from ..core.trainer import DOMAIN_DATASET_MODES, get_dataset


def get_training_dataset(domain, data_root):
    mode = DOMAIN_DATASET_MODES[domain]
    return get_dataset(domain, data_root, "train", mode)


def get_output_dir(out_dir, domain):
    return Path(out_dir) if out_dir else Path("out") / "objective_contrast" / domain
...
def main():
    ...
    p.add_argument("--domain", default="protein", choices=list(DOMAIN_DATASET_MODES))
    ...
    ds = get_training_dataset(args.domain, args.data_root)
    results = {}
    ...
    out = get_output_dir(args.out_dir, args.domain)
